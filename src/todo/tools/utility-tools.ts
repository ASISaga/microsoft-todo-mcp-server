/**
 * UtilityTools – MCP tool registrations for bulk operations and diagnostics.
 *
      ...
      async ({ sourceListId, targetListId, olderThanDays, dryRun }) => {
        try {
          ...
          for (const task of tasksToArchive) {
            try {
              const createResponse = await this.graphClient.request(
      ...
      async ({ testType }) => {
        try {
          ...
          if (testType === "odata-select" || testType === "all") {
            ...
            try {
              const response = await this.graphClient.request<{ value: Record<string, unknown>[] }>(
          ...
          if (testType === "extensions" || testType === "all") {
            ...
            try {
              ...
              if (listsResponse && listsResponse.value && listsResponse.value.length > 0) {
                ...
                try {
                  const extResponse = await this.graphClient.request(
          ...
          if (testType === "all") {
            ...
            for (const endpoint of endpoints) {
              try {
                const response = await this.graphClient.request(`${MS_GRAPH_BASE}${endpoint}`)
