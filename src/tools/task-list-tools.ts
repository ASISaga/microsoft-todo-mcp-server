/**
 * Task-list tools – MCP tool registrations for Microsoft To Do task lists.
 *
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const response = await graphClient.request<{ value: TaskList[] }>(`${MS_GRAPH_BASE}/me/todo/lists`, token)
  ...
  server.tool(
    ...
    async ({ displayName }) => {
      try {
        ...
        const response = await graphClient.request<TaskList>(`${MS_GRAPH_BASE}/me/todo/lists`, token, "POST", {
          displayName,
        })
  ...
  server.tool(
    ...
    async ({ listId, displayName }) => {
      try {
        ...
        const response = await graphClient.request<TaskList>(
