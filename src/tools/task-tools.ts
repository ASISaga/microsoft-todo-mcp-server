/**
 * Task tools – MCP tool registrations for Microsoft To Do tasks.
 *
  ...
  server.tool(
    ...
    async ({ listId, filter, select, orderby, top, skip, count }) => {
      try {
        ...
        const response = await graphClient.request<{ value: Task[]; "@odata.count"?: number }>(url, token)
  ...
  server.tool(
    ...
    async ({
      ...
      try {
        ...
        const response = await graphClient.request<Task>(
