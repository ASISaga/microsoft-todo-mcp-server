/**
 * GitHub tools – MCP tool registrations for GitHub ↔ Microsoft To Do integration.
 *
...
export function registerGitHubTools(server: McpServer): void {
  server.tool(
    ...
    async ({ listId, taskId }) => {
      try {
        const githubToken = gitHubClient.getToken()
        if (!githubToken) {
          return {
            content: [
              {
        ...
        const task = await graphClient.request<Task>(
        ...
        await graphClient.request<Task>(`${MS_GRAPH_BASE}/me/todo/lists/${listId}/tasks/${taskId}`, msToken, "PATCH", {
          ...
          for (const task of tasksResponse.value) {
            ...
            try {
              ...
              if (issue && issue.state === "closed") {
                await graphClient.request<Task>(
